...
> ⚠️ **No authentication, and the Docker image binds `0.0.0.0`.** Running `docker compose up -d` or `docker run -p 12345:12345 swissknife-mcp` exposes every tool to anything that can reach the published port — including the `http` / `dns` / `inspect` outbound primitives. Keep the port bound to `127.0.0.1` (`-p 127.0.0.1:12345:12345`), put it behind a proxy/VPN, or — at minimum — set `SWISSKNIFE_BLOCK_PRIVATE_NETWORKS=1` so the server can't be used to probe your internal network. See [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md) for the full hosting checklist.
