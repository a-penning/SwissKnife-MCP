...
| `SWISSKNIFE_MAX_CONCURRENT_WAITS` | `8` | Max simultaneous `wait` calls — each holds its request open for the requested duration. Excess calls are **rejected** (not queued). Global count; the stateless server can't scope it per client. |
...
1. **build stage** — `node:26-alpine` (pinned by multi-arch digest), `npm ci`, `npm run build`
2. **runtime stage** — `node:26-alpine`, copy `dist/` + production `node_modules`, run as non-root `node` user, `HEALTHCHECK` against `/healthz`, `EXPOSE 12345`. The image sets `HOST=0.0.0.0` so the container is reachable through a published port.

```bash
docker build -t swissknife-mcp .
# Publish to loopback only (the compose default); drop the 127.0.0.1 prefix
# to expose on all interfaces — only behind an authenticating proxy.
docker run -d --name swissknife -p 127.0.0.1:12345:12345 swissknife-mcp
# or
docker compose up -d   # binds 127.0.0.1 by default
```
...
SwissKnife speaks the **Streamable HTTP** transport. Clients that support it (Claude Code, Cursor, and others) point straight at the `/mcp` endpoint:

```json
// Claude Code (.mcp.json) — HTTP server
{
  "mcpServers": {
    "swissknife": {
      "type": "http",
...
### Clients without HTTP support — stdio bridge

Some clients only speak the **stdio** transport — most notably **Claude Desktop**, configured via `claude_desktop_config.json`. Bridge them to the HTTP server with [`mcp-remote`](https://www.npmjs.com/package/mcp-remote), a small stdio↔HTTP proxy that `npx` fetches on demand:

```json
// claude_desktop_config.json — stdio client bridged to the HTTP server
{
  "mcpServers": {
    "swissknife": {
      "command": "npx",
...
Restart the client after editing. `mcp-remote` spawns one proxy process per server and forwards stdio JSON-RPC to `POST /mcp`, so the server itself still only ever sees HTTP — the SSRF guard, `Host`/`Origin` allow-lists, and rate limit all apply unchanged. The server must already be running (`docker compose up -d` or `npx .`) before the client launches the bridge.
