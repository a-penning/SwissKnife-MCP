 ...
 * With it on it does two things: an eager `assertUrlAllowed` so a blocked target
 * fails with a precise, transport-independent message, and a dispatcher whose
 * connector re-screens and dials the resolved IP in one step — the authoritative
 * gate that closes the rebinding TOCTOU even if the record flips after the eager
 * check. TLS SNI and certificate validation still use the URL hostname.
 *
 * (The eager check matters because undici surfaces a connector error as an
 * opaque `fetch failed` whose cause shape varies by version; the explicit
 * assert keeps the SSRF reason stable for callers.)
 */
export async function guardedFetch(
  ...
  await assertUrlAllowed(typeof url === "string" ? new URL(url) : url);
  return fetch(url, {
    ...init,
