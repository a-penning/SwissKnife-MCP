...
export const idTool = defineTool({
  ...
  inputSchema: {
    ...
    uppercase: coerceBoolean(true).describe(
      "password: include uppercase letters.",
    ),
    digits: coerceBoolean(true).describe("password: include digits."),
    symbols: coerceBoolean(true).describe(
      "password: include symbols (set: !@#$%^&*()-_=+[]{};:,.<>?).",
    ),
    value: singleOrBatch
      .optional()
      .describe(
