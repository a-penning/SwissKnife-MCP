...
export const jwtTool = defineTool({
  ...
  inputSchema: {
    ...
    algorithms: singleOrArray(z.string())
      .optional()
      .describe(
        "verify: allowed algs (default: any). A single string or an array.",
      ),
    ...
    payload: jsonObjectArg(z.record(z.string(), z.unknown()))
      .optional()
      .describe(
        "sign: JWT claims (sub / iss / aud / jti / nbf / iat / exp etc. — these are payload keys, not separate args). Object, or a JSON-stringified object.",
      ),
    expiresIn: z
      .string()
      .optional()
    ...
    header: jsonObjectArg(z.record(z.string(), z.unknown()))
      .optional()
      .describe(
        "sign: extra header fields (object, or a JSON-stringified object)",
      ),
  },
  refine: (args, ctx) => {
    if (args.action === "sign") {
        ...
        case "verify": {
          ...
          try {
            ...
            const { payload, protectedHeader } = await jwtVerify(
              ...
              {
                ...(args.algorithms
                  ? { algorithms: [args.algorithms].flat() }
                  : {}),
